...
/*
This test relies on an index generated by FacetTest.testThreeFileIngest() from
the Control-module.
 */
...
/*    public void testStrings() throws Exception {
    ...
    public void testSimpleWarmup() throws Exception {
        makeIndex();
        Configuration conf = Configuration.load(basicSetup().getAbsolutePath());
        conf.set(SearchNodeImpl.CONF_WARMUP_DATA, "warmup_simple.dat");
        SummaSearcherImpl searcher = new SummaSearcherImpl(conf);
        Request request = new Request();
        request.put(DocumentKeys.SEARCH_QUERY, "*");
        String result = searcher.search(request).toXML();
    ...
    public void testComplexWarmup() throws Exception {
        makeIndex();
        Configuration conf = Configuration.load(basicSetup().getAbsolutePath());
        conf.set(SearchNodeImpl.CONF_WARMUP_DATA, "warmup_complex.dat");
        SummaSearcherImpl searcher = new SummaSearcherImpl(conf);
        Request request = new Request();
        request.put(DocumentKeys.SEARCH_QUERY, "*");
        String result = searcher.search(request).toXML();
...
//        File indexLocation = new File("myhome/myindexfolder");
        ...
        IndexReader[] subs = reader.getSequentialSubReaders() == null ? 
