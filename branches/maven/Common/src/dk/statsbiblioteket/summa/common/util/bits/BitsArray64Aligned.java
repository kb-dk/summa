 ...
 * representation is that of {@link BitsArray64Aligned} with the notable
 * difference that the amount of bits allocated for values are always 1, 2, 4,
 * 8, 16 or 32 so that the values always fit inside a single long.
 * </p><p>
...
@QAInfo(level = QAInfo.Level.NORMAL,
        ...
        author = "te")
public class BitsArray64Aligned extends BitsArray64Impl {
     ...
     */
    public BitsArray64Aligned(int length, int maxValue) {
        ...
        log.trace("Creating BitsArrayAligned of length " + length
                  ...
                  + maxValue);
        setAttributes(bits, 0, new long[length * bits / BLOCK_SIZE + 2]);
    }
    ...
    protected void ensureSpace(final int position, final int value) {
        if (position > maxPos || value > maxValue) {
            ...
            BitsArray64Aligned array;
            if (position > maxPos) {
                array = new BitsArray64Aligned(
    ...
    public void assign(final BitsArray other) {
        ...
        BitsArray64Aligned bap = (BitsArray64Aligned)other;
        clear();
