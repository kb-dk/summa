...
public class PoolFactoryGate {
  ...
  public static synchronized SimplePair<CollectorPool, TagCollector> acquire(
    ...
    try {
      ...
      if (!hasPool) {
        log.info("The CollectorPoolFactory has no structures for the given " +
                 "request from " + caller + ". A new structure will be " +
                 "generated, which can take several minutes. The request was "
                 + request.getBuildKey());
      }
      ...
      if (!hasPool) {
        log.info("The PoolFactory was successfully updated in "
                 + buildTime/1000 + " seconds. "
    ...
    try {
      ...
      switch (availability) {
        ...
        case hasFilled:
          ...
          break;
        case mustWait:
          log.debug("All allowed TagCollectors are active for key '" + key
                    + "' for " + caller);
          break;
        case mustCreateNew:
          log.info("A new TagCollector will be created for key '" + key
                   + "' for " + caller + " from " + collectorPool);
          break;
        case mightCreateNew:
      ...
      if (tagCollector.isNewborn()) {
        ...
        if (a % 100 == 0 || log.isDebugEnabled()) {
          String message = "Allocated " + tagCollector + " #" + a
                           ...
                           + factory.getMem()/1048576 + "MB with MachineStats("
                           + MachineStats.stats() + ")";
          if (a % 100 == 0) {
            log.info(message);
          } else {
      ...
      throw new OutOfMemoryError(
          "Encountered OOM when acquiring TagCollector for '" + key
          + "' with full request " + request + " from pool " + collectorPool
          + " with full factory " + factory + "\nCaused by: "
          + writer.toString());
    }
  ...
  public static void release(CollectorPool pool, TagCollector collector, String key) {
    ...
    if (freed) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "The release of a TagCollector with size "
            + collector.getMemoryUsage()/1048576
