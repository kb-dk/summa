...
public class PoolFactoryGate {
   ...
   */
  public static synchronized SimplePair<CollectorPool, ? extends TagCollector> acquire(
    ...
    try {
      ...
      if (!hasPool) {
        log.info("The CollectorPoolFactory has no structures for the given request from " + caller + ". A new structure"
                 + " will be generated, which can take several minutes. The request was " + request.getBuildKey());
      }
      ...
      if (!hasPool) {
        log.info("The PoolFactory was successfully updated in " + buildTime/1000 + " seconds. "
                 + "Total allocation is " + factory.toString() + " with MachineStats(" + MachineStats.stats() + ")");
      }
    } catch (IOException e) {
    ...
    try {
      ...
      switch (availability) {
        ...
        case mustWait:
          ...
          break;
        case mustCreateNew:
          log.info("A new TagCollector will be created for key '" + key + "' for " + caller + " from " + collectorPool);
          break;
        case mightCreateNew:
      ...
      if (tagCollector.isNewborn()) {
        ...
        if (a % 100 == 0 || log.isDebugEnabled()) {
          String message =
              "Allocated " + tagCollector + " #" + a + "(availability was " + availability + "). "
              + "CollectorPoolFactory memory allocation is now " + factory.getMem()/1048576 + "MB with MachineStats("
              + MachineStats.stats() + ")";
          if (a % 100 == 0) {
            log.info(message);
          } else {
          ...
          log.debug("CollectorPoolFactory structure allocation is "
                    + factory.toString());
        }
      }
      ...
      throw new OutOfMemoryError(
          "Encountered OOM when acquiring TagCollector for '" + key + "' with full request " + request + " from pool "
          + collectorPool + " with full factory " + factory + "\nCaused by: " + writer.toString());
    }
  ...
  public static void release(CollectorPool pool, TagCollector collector, String key) {
    ...
    if (freed) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "The release of a TagCollector with size " + collector.getMemoryUsage()/1048576
