 ...
 * newspaper scans with dates sich as 1868-04-31, which seems like an oversight and should be rolled to 1868-05-01.
 * </p><p>
...
public class SolrLenientTimestamp implements SolrDocumentAdjustFilter.Adjuster {
    ...
    public boolean adjust(Record record) {
        ...
        while (fieldMatcher.find()) {
            ...
            adjusted |= !cDate.equals(fieldMatcher.group(4));
        }
        if (adjusted) {
    ...
    private String correctDate(Record record, String timestamp) {
        if (timestamp.length() != 24) {
            Logging.logProcess("SolrLenientTimestamp", "Unsupported timestamp '" + timestamp + "'",
        ...
        return String.format("%04d-%02d-%02dT%s", year, month, day, timestamp.substring(11));
    }
