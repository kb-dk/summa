...
public class SBMARCParser extends MARCParser {
     ...
     */
    public static final String CONF_STATUS_FIELD = "sbmarcparser.status.field";
     ...
     */
    public static final String CONF_STATUS_SUBFIELD = "sbmarcparser.status.subfield";
     ...
     */
    public static final String CONF_STATUS_DELETED = "sbmarcparser.status.deleted";
     ...
     */
    private MarcAnnotations.MultiVolumeType type = MarcAnnotations.MultiVolumeType.NOTMULTI;
    /**
    ...
    public SBMARCParser(Configuration conf) {
        ...
        statusDeletedCode = conf.getString(CONF_STATUS_DELETED, statusDeletedCode);
        log.debug("Constructed SBMARCParser with deleted status " + statusField + statusSubfield + "="
                  + statusDeletedCode);
        initializeNewParse();
    }
    ...
    public class Child implements Comparable<Child> {
        ...
        public int compareTo(Child other) {
            ...
            return other.sortField == null ? 1 : sortField.compareTo(other.sortField);
        }
    ...
    @Override
    protected void setSubField(String dataFieldTag, String dataFieldInd1, String dataFieldInd2, String subFieldCode,
                               String subFieldContent) {
        // ID
        if (ID_FIELD.equals(dataFieldTag) && ID_FIELD_SUBFIELD.equals(subFieldCode)) {
...
/*            log.trace("Setting id from " + ID_FIELD_SUBFIELD + " to '"
        ...
        if (SBID_FIELD.equals(dataFieldTag) && SBID_FIELD_SUBFIELD.equals(subFieldCode)) {
            setID(expandID(subFieldContent), 1); // Highest priority
            log.trace("Setting id from " + SBID_FIELD_SUBFIELD + " to '" + subFieldContent
                      + "' gave processed id '" + id + "'");
            return;
        }

        // Status and type
        if (statusField.equals(dataFieldTag)) {
            log.trace("Entered " + dataFieldTag + " in " + id + " with subfield " + subFieldCode);
        ...
        if (CHILD_FIELD.equals(dataFieldTag)) {
            if (CHILD_FIELD_SUBFIELD_ID.equals(subFieldCode) || CHILD_FIELD_SUBFIELD_SBID.equals(subFieldCode)) {
     ...
     */
    @Override
    protected String beginSubFieldTagToString(
            XMLStreamReader reader, String tag, String ind1, String ind2, String code) {
        String replaceSrcCode;
        String replaceDestCode;
        if (PARENT_FIELD.equals(tag) && PARENT_FIELD_SUBFIELD_SBID.equals(code)) {
            replaceSrcCode = PARENT_FIELD_SUBFIELD_SBID;
            replaceDestCode = PARENT_FIELD_SUBFIELD_ID;
        } else if (CHILD_FIELD.equals(tag) && CHILD_FIELD_SUBFIELD_SBID.equals(code)) {
            replaceSrcCode = CHILD_FIELD_SUBFIELD_SBID;
            replaceDestCode = CHILD_FIELD_SUBFIELD_ID;
        } else {
            return super.beginSubFieldTagToString(reader, tag, ind1, ind2, code);
        }

        StringWriter xml = new StringWriter(50);
        xml.append("<").append(reader.getLocalName());
        for (int i = 0; i < reader.getAttributeCount(); i++) {
            String attributeLocalName = replaceSrcCode.equals(reader.getAttributeLocalName(i)) ?
                                        replaceDestCode :
    ...
    protected Record makeRecord(String xml) {
        ...
        if (type != MarcAnnotations.MultiVolumeType.NOTMULTI) {
            //noinspection DuplicateStringLiteralInspection
            log.debug("Marking '" + id + "' as " + type + " with " + (parent == null ? "no parent" : "parent " + parent)
