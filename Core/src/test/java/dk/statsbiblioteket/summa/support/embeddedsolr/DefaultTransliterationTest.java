...
public class DefaultTransliterationTest {
	...
	public void testVoidTransliteration() throws Exception {
		...
		String[] files = new String[]{
				"support/solr_test_documents/default_transliteration_doc.txt",
		
		};
		SolrServerUnitTestUtil.indexFiles(files);
		SolrQuery query = new SolrQuery("cplusplus");
		QueryResponse response = solrServer.query(query);
		assertEquals(1L, response.getResults().getNumFound());

		query = new SolrQuery("c++");
