...
public class RelationResolverTest extends TestCase {
    ...
    private File NONMATCHED = new File("target/RelationResolverTest/relations/nonmatched");
    ...
    public void testBasicEnrichment() throws IOException {
        ...
        Storage storage = StorageFactory.createStorage(Configuration.newMemoryBased(
                Storage.CONF_CLASS, H2Storage.class,
                DatabaseStorage.CONF_LOCATION, STORAGE.getAbsolutePath()));
        storage.flush(parentR);
        assertNotNull("The parent Record should be stored", storage.getRecord(parentR.getId(), null));

        Filter feeder = new PayloadFeederHelper(Arrays.asList(new Payload(enricherR)));
        ...
        assertTrue("The RelationResolver should have a Payload", resolver.hasNext());
        Payload enriched = resolver.next();
        assertNotNull(
                "The enriched " + enriched.getRecord() + " should have parents", enriched.getRecord().getParentIds());
        assertEquals("The enriched Payload should have the right parent", "parent1",
                     enriched.getRecord().getParentIds().get(0));

        storage.flush(enriched.getRecord());

        Record finalParent = storage.getRecord("parent1", new QueryOptions(null, null, 10, 0));
        assertNotNull("The final parent should have a child", finalParent.getChildren());
        assertEquals("The final parent should have the right child", "enricher1",
                     finalParent.getChildren().get(0).getId());
        storage.close();
    }
    ...
    private final String CONTENT10 = "(?s).*<publicationStmt.*?>.+?<idno.+?type=\"ISBN\">[^0-9]*([0-9])[^0-9]*([0-9])"
                                     ...
                                     + "([0-9])[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*?</idno>.*";
    @SuppressWarnings({"FieldCanBeLocal"})
    private final String CONTENT13 = "(?s).*<publicationStmt.*?>.+?<idno.+?type=\"ISBN\">[^0-9]*([0-9])[^0-9]*([0-9])"
                                     + "[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*"
                                     + "([0-9])[^0-9]*([0-9])[^0-9]*([0-9])[^0-9]*?([0-9])[^0-9]*([0-9])[^0-9]*"
                                     + "([0-9])[^0-9]*?</idno>.*";
    @SuppressWarnings({"FieldCanBeLocal"})
    private final String ORIGIN10 = ".*[^0-9/]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])"
                                    + "[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*"
                                    + "([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*[^/]*$";
    @SuppressWarnings({"FieldCanBeLocal"})
    private final String ORIGIN13 = ".*[^0-9/]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])"
                                    + "[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*"
                                    + "([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])"
                                    + "[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*([0-9])[^0-9/a-zA-Z]*[^/]*$";
    ...
    public void testRegexprs() {
        ...
        testRegexprs(ORIGIN10, GROUP10, "somefolder/somezip7!/1234 567 890 - Svamperiget 876.xml", "1234567890");
        testRegexprs(ORIGIN13, GROUP13, "somefolder/somezip7!/1234 567 890-123 - Svamperiget 876.xml", "1234567890123");
    }

    public void testRegexprs(String regexp, String template, String input, String expected) {
        Pattern pattern = Pattern.compile(regexp);
        Matcher matcher = pattern.matcher(input);
        assertTrue("The pattern '" + regexp + "' should match input '" + input + "'", matcher.matches());
        ...
        assertEquals("The result from pattern '" + regexp + "' and input '" + input
                     + "' should be as expected", expected, actual);
    }
    ...
    public void testShapeEnrichIngest() throws IOException {
        ...
        assertNotNull("The parent Records should be stored", storage.getRecord(parentR1.getId(), null));

        Payload teiP1 = new Payload(teiR1);
        teiP1.getData().put(Payload.ORIGIN, "somefolder/somezip7!/09876 543 21 - Svamperiget 8.xml");
        Payload teiP2 = new Payload(teiR2);
        teiP2.getData().put(Payload.ORIGIN, "somefolder/somezip7!/09876 543 21 - 098 - Svamperiget 8.xml");

        ObjectFilter feeder = new PayloadFeederHelper(Arrays.asList(teiP1, teiP2));
        ...
        ObjectFilter resolver = new RelationResolver(resolverConf) {
            ...
            protected DocumentResponse getHits(Payload payload, String searchValue) throws PayloadException {
                ...
                if ("0987654321".equals(searchValue)) {
                    response = createResponse(searchValue, "internal_id1");
                } else if ("1234567890".equals(searchValue)) {
                    response = createResponse(searchValue, "internal_id2", "internal_id3");
                } else if ("0987654321098".equals(searchValue)) {
                    response = createResponse(searchValue, "internal_id1");
                } else if ("1234567890123".equals(searchValue)) {
                    response = createResponse(searchValue, "internal_id2");
                } else {
                    response = new DocumentResponse(null, searchValue, 0, 2, "LUCENE", false,
        ...
        assertEquals("The correct number of Records should be pulled through the chain", 2, counter);
        writer.close(true);

        Record enriched1 = storage.getRecord(teiR1.getId(), new QueryOptions(null, null, 10, 10));
        assertNotNull("The enriching Record1 should be stored", enriched1);
        assertNotNull("The enriching Record1 should have parents", enriched1.getParents());
        assertEquals("The enriching Record1 should have the right parent count", 3, enriched1.getParents().size());

        Record enriched2 = storage.getRecord(teiR2.getId(), new QueryOptions(null, null, 10, 10));
        assertNotNull("The enriching Record2 should be stored", enriched2);
        assertNotNull("The enriching Record2 should have parents", enriched2.getParents());
        assertEquals("The enriching Record2 should have the right parent count", 2, enriched2.getParents().size());
        storage.close();
    }

    private DocumentResponse createResponse(String searchValue, String... ids) {
        DocumentResponse response = new DocumentResponse(null, searchValue, 0, 2, "LUCENE", false,
    ...
    public void xtestSearcherConnection() throws IOException {
        ...
        assertTrue("The RelationResolver should provide at least one Payload", resolver.hasNext());
        Payload enriched = resolver.next();
        assertNotNull("The enriched " + enriched.getRecord() + " should have parents",
