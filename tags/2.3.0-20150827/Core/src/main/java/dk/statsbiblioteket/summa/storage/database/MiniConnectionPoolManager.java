...
public class MiniConnectionPoolManager {
    ...
    public MiniConnectionPoolManager(ConnectionPoolDataSource dataSource, int maxConnections, int timeout) {
        ...
        log.debug("Created for source " + dataSource.getClass() + " and max connections " + maxConnections
                  + " and timeout " + timeout);
    }
    ...
    public synchronized void dispose() throws SQLException {
        ...
        while (!recycledConnections.isEmpty()) {
            ...
            try {
                ...
                log.warn("Error disposing of " + pconn + ": " + e.getMessage(), e);
            }
        }
    }
     ...
     */
    public PreparedStatement getManagedStatement(StatementHandle handle) throws SQLException {
        PooledConnection pconn = getPooledConnection();
        Connection conn = pconn.getConnection();

        if (log.isTraceEnabled()) {
            log.trace("Getting statement for handle " + handle + " on connection " + pconn.hashCode());
        }
    ...
    protected PooledConnection getPooledConnection() {
        ...
        if (log.isTraceEnabled()) {
            log.trace("Getting pooled connection, with " + getActiveConnections() + " connections currently active");
        }
    ...
    private synchronized void recycleConnection(PooledConnection pconn) {
        ...
        if (activeConnections <= 0) {
            throw new AssertionError("Can not recycle " + pconn.hashCode() + ". No connections registered as active");
        }
    ...
    private synchronized void disposeConnection(PooledConnection pconn) {
        if (activeConnections <= 0) {
            throw new AssertionError("Can not dispose " + pconn.hashCode() + ". No connections registered as active");
        }
    ...
    private void closeConnectionNoEx(PooledConnection pconn) {
        try {
            ...
            log.warn("Error while closing database connection: " + e.getMessage(), e);
        }
    }
    ...
    private class PoolConnectionEventListener implements ConnectionEventListener {
        ...
        public void connectionClosed(ConnectionEvent event) {
            ...
            if (log.isTraceEnabled()) {
                log.trace("Handled ConnectionClosedEvent for " + pconn.hashCode() + ", now " + getActiveConnections()
                          + " active connections");
            }
        }
        ...
        public void connectionErrorOccurred(ConnectionEvent event) {
            ...
            log.info("Handled ConnectionErrorEvent, now " + getActiveConnections() + " active connections. "
                     + "Error was: " + event.getSQLException().getMessage(), event.getSQLException());
        }
    }
