...
public class RecordGenerator implements ObjectFilter {
     ...
     */
    public static final String CONF_CONTENT_TEMPLATE_LOCATION = "summa.ingest.generator.template.location";
     ...
     */
    public static final String CONF_CONTENT_TEMPLATE = "summa.ingest.generator.template";
     ...
     */
    public static final String CONF_BASE_TEMPLATE = "summa.ingest.generator.base";
     ...
     */
    public static final String CONF_ID_TEMPLATE = "summa.ingest.generator.idtemplate";
    public static final String DEFAULT_ID_TEMPLATE = "dummy_$TIMESTAMP[ms]_$INCREMENTAL_NUMBER[id]";
     ...
     */
    public static final String CONF_DETERMINISTIC_RANDOM = "summa.ingest.generator.random.deterministic";
    ...
    public RecordGenerator(Configuration conf) throws ConfigurationException {
        ...
        boolean deterministic = conf.getBoolean(CONF_DETERMINISTIC_RANDOM, DEFAULT_DETERMINISTIC_RANDOM);
    ...
    private List<RecordToken> parseTemplate(String template) {
        ...
        while (matcher.find()) {
            ...
            if (matcher.start(1) > lastEnd) {
                tokens.add(new LiteralToken(template.substring(lastEnd, matcher.start(1))));
            }
            for (RecordToken tokenFactory : factories) {
                RecordToken token = tokenFactory.createTokenIfMatch(matcher.group(1));
                if (token != null) {
                    tokens.add(token);
                    break;
                }
        ...
        if (lastEnd != template.length()) {
            tokens.add(new LiteralToken(template.substring(lastEnd, template.length())));
        }
    ...
    class IncrementalNumberToken implements RecordToken {
        ...
        @Override
        public RecordToken createTokenIfMatch(String template) {
            Matcher incrementalNumber = PATTERN_INCREMENTAL_NUMBER.matcher(template);
    ...
    class RandomIntToken implements RecordToken {
        ...
        @Override
        public RecordToken createTokenIfMatch(String template) {
            Matcher matcher = PATTERN_RANDOM_NUMBER.matcher(template);
            if (!matcher.matches()) {
                return null;
            }
    ...
    class RandomCharsToken implements RecordToken {
        ...
        public RandomCharsToken(int min, int max, boolean onlyLetters) {
            log.trace("Creating RandomCharsToken(" + min + ", " + max + ", " + onlyLetters + ")");
            this.min = min;
            this.max = max;
        ...
        @Override
        public RecordToken createTokenIfMatch(String template) {
            Matcher matcher = PATTERN_RANDOM_CHARS.matcher(template);
            if (!matcher.matches()) {
                return null;
            }
            int min = Integer.parseInt(matcher.group(2));
    ...
    class RandomWordsToken implements RecordToken {
        ...
        public RandomWordsToken(int min, int max, int minLength, int maxLength, boolean onlyLetters) {
            log.trace("Creating RandomWordsToken(" + min + ", " + max + ", " + minLength + ", " + maxLength + ", "
                      + onlyLetters + ")");
            this.min = min;
            this.max = max;
        ...
        @Override
        public RecordToken createTokenIfMatch(String template) {
            Matcher matcher = PATTERN_RANDOM_WORDS.matcher(template);
            if (!matcher.matches()) {
                return null;
            }
            int min = Integer.parseInt(matcher.group(2));
            ...
            return new RandomWordsToken(min, max, minLength, maxLength, onlyLetters);
        }
    }
    ...
    class WordListToken implements RecordToken {
        ...
        public WordListToken(int min, int max, String listName) {
            log.trace("Creating WordListToken(" + min + ", " + max + ", " + listName + ")");
            this.min = min;
            this.max = max;
        ...
        @Override
        public RecordToken createTokenIfMatch(String template) {
            Matcher matcher = PATTERN_WORD_LIST.matcher(template);
            if (!matcher.matches()) {
                return null;
            }
            int min = Integer.parseInt(matcher.group(2));
    ...
    public Payload next() {
        ...
        if (sinceLast < minDelay) {
            ...
            try {
                ...
                log.warn("Interrupted while sleeping " + (minDelay - sinceLast) + " ms. Continuing");
            }
        }
    ...
    private List<String> getWords(String listName) {
        ...
        if (result == null) {
            ...
            if (result == null) {
                throw new IllegalArgumentException("The word-list '" + listName + "' was requested by the"
