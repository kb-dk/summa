...
popd > /dev/null
#svn add --force --depth infinity $LUCENE
...
rm -r java
popd > /dev/null
#svn add --force --depth infinity $SOLR

# We need to prune .svn from the build files
TMP=tmp_delete
rm -rf $TMP
mkdir $TMP
...
echo "Patching build-files for Lucene"
# Insert appropriate calls in the common-build.xml for Lucene
pushd $LUCENE > /dev/null
cd ..
# Patch generated by calling 'svn diff module-build.xml > patch_module-build'
patch -N module-build.xml patch_module-build 
popd > /dev/null

echo "Patching build-files for Solr"
# Insert appropriate calls in the build.xml and common-build.xml for solr
pushd $SOLR > /dev/null
cd ../..
# lucene_4/solr
# Patch generated by calling 'svn diff common-build.xml > patch_common-build'
...
# Patch generated by calling 'svn diff solrconfig.xml > patch_solrconfig'
patch -N solrconfig.xml patch_solrconfig
popd > /dev/null

# We add the copied files to svn
cd lucene_4
