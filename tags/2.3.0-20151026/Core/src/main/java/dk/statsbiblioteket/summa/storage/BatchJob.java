 ...
 * <li><tt>base</tt> - The name of the base being operated on - possibly {@code null} when iterating over all bases</li>
 * <li><tt>minMtime</tt> - The minimum modification time for the records in the batch</li>
 * <li><tt>maxMtime</tt> - The maximum modification time for the records in the batch</li>
 * <li><tt>options</tt> - A {@link QueryOptions} instance</li>
 * <li><tt>state</tt> - A private variable to be used by the script to hold arbitrary state in between invocations.
 *                      This variable is initialized to {@code null} by the runtime</li>
 * <li><tt>out</tt> - A {@link PrintWriter} collecting any output to return from the script. Notably methods
 *                    {@code out.print()} and {@code out.println()} are available</li>
 *  <li><tt>commit</tt> - A boolean flag that must be set to {@code true} if the script wants to commit any changes it
 *    has done to {@code record}. This variable will be set to {@code false} prior to each evaluation of the script</li>
 * <li><tt>first</tt> - A boolean flag which is {@code true} on the first invocation of the script.
 *                      Always {@code false} otherwise</li>
 * <li><tt>last</tt> - A boolean flag which is {@code true} when the last record in the batch is reached.
 *                     Always {@code false} otherwise</li>
 * </ul>
 *
 * @author mke
...
public class BatchJob {
    ...
    /**
     * Create a new batch job. Before running the job with {@link #eval()} eval be sure to set the job context to
     * operate on with {@link #setContext(Record, boolean, boolean)}.
     *
     * @param jobName  The name of the job to instantiate. The job name must match the regular expression
     *                 {@code [a-zA-z_-]+.job.[a-zA-z_-]+} and correspond to a resource in the classpath.
     *                 Fx {@code count.job.js}
     * @param log      Inserted into the namespace of the batch script as {@code log}
     * @param base     Inserted into the namespace of the batch script as {@code base}
     * @param minMtime Inserted into the namespace of the batch script as {@code minMtime}
     * @param maxMtime Inserted into the namespace of the batch script as {@code minMtime}
     * @param options  Inserted into the namespace of the batch script as {@code options}
     * @throws IOException              if there is an error loading the resource
     * @throws ScriptException          if there is an error parsing or compiling the script
     * @throws IllegalArgumentException if {@code jobName} doesn't match the required regular expression
     * @throws FileNotFoundException    if the resource {@code jobName} was not found in the classpath
     */
    public BatchJob(String jobName, Log log, String base, long minMtime, long maxMtime,
                    QueryOptions options) throws IOException, ScriptException {
     ...
     * @param first  whether or not this is the first execution of the batch script
     ...
     *
     * @return the name of the system resource from which the batch script was loaded
     */
