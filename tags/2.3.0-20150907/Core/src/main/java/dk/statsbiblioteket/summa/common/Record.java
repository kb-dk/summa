...
public class Record implements Serializable, Comparable {
    ...
    public enum ValidationState {
        ...
        public static ValidationState fromString(String in) {
            if ("notValidated".equals(in)) {
                ...
                log.warn("Unknown state: '" + in + "' in ValidationState.fromString. Returning " + notValidated);
                return notValidated;
            }
        }
    }
    ...
    public Record(String id, String base, byte[] data, long lastModified) {
        init(id, base, false, true, data, 0, lastModified, null, null, null, false);
    }
     ...
     */
    public Record(String id, String base, boolean deleted, boolean indexable, byte[] data, long creationTime, 
                  long lastModified, List<String> parentIds, List<String> childIds, StringMap meta, 
                  boolean contentCompressed) {
        init(id, base, deleted, indexable, data, creationTime, lastModified, parentIds, childIds, meta, 
             contentCompressed);
    }
    ...
    public void init(String id, String base, boolean deleted, boolean indexable, byte[] data, long creationTime, 
                     long lastModified, List<String> parents, List<String> children, StringMap meta, 
                     boolean contentCompressed) {
        setId(id);
        setBase(base);
        setDeleted(deleted);
    ...
    public void setBase(String base) {
        ...
        if ("".equals(base)) {
            throw new IllegalArgumentException("base must not be the empty string for record '" + getId() + "'");
        }
        this.base = base;
    }
...
/*            throw new IllegalArgumentException("data must be specified for "
    ...
    public void setChildIds(List<String> childIds) {
        if (childIds == null) {
            ...
            //noinspection DuplicateStringLiteralInspection
            log.warn("No childIds should be stated by null, not the empty list. Problematic Record with id '"
    ...
    @QAInfo(level = QAInfo.Level.FINE,
            state = QAInfo.State.QA_NEEDED,
            comment = "Hans made the hash-function, so no present persons know how it works (or if)")
    public int hashCode() {
        int result;
    ...
    public boolean equals(Object o) {
        ...
        try {
            return id.equals(other.getId()) && base.equals(other.getBase())
                   ...
                   && deleted == other.isDeleted() && indexable == other.isIndexable() && (
                    (parentIds == null && other.getParentIds() == null) || (parentIds != null
                                                                            && deepEquals(parentIds, 
                                                                                          other.getParentIds())))
                   && Arrays.equals(getContent(), other.getContent()) && (
                    ...
                    || (childIds != null) && deepEquals(childIds, other.getChildIds())) && (
                           (!hasMeta() == !other.hasMeta()) || (hasMeta() && getMeta().equals(other.getMeta())));
        } catch (Exception e) {
            log.error("Error calling equals for " + this + " and " + other + ". Returning false", e);
            return false;
        }
    }
    ...
    public String toString(boolean verbose) {
        return "Record [id(" + getId() + "), base(" + getBase() + "), deleted(" + isDeleted() + "), indexable("
               ...
               + "), num-parentsIDs(" + (parentIds == null ? 0 : parentIds.size()) + "), num-childRecords(" + (
                children == null ? 0 : children.size()) + "), num-parentRecords(" 
