...
#
# Suitable for auto generating launcher scripts. 
#
# Directory Structure:
...
#
#    LIBDIRS          # Optional. Space separated list of paths in where to look 
#                     # for jar files. Defaults to ./lib
#
#    PRINT_CONFIG     # Optional. Print config to stderr. Set to empty to not print config.
#
#    JAVA_HOME        # Optional. If there's a global JAVA_HOME it will be used by default.
#
...
#
# RMI Codebase:
#    CODEBASE_BASEURL # The base url from which libs can be fetched
#
...
pushd lib > /dev/null
CODEBASE="-Djava.rmi.server.codebase=$CODEBASE_BASEURL/$(ls summa-common-*.jar) $CODEBASE_BASEURL/$(ls summa-storage-api-*.jar)"
popd
...
if [ ! -z $PRINT_CONFIG ]; then 
    ...
    echo -e "Codebase:\tFIXME-DISABLED: $CODEBASE" 1>&2
    echo -e "Jvm options:\t$JVM_OPTS\n" 1>&2
fi

# The CODEBASE is disbled because it doesn't work properly with an external registry
