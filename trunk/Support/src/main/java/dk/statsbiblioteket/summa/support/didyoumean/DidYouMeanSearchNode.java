...
public class DidYouMeanSearchNode extends SearchNodeImpl {
    ...
    /** Log factory. */
    private static Log log = LogFactory.getLog(DidYouMeanSearchNode.class);
    ...
    /**
     * The configuration file in the configuration file. This is a boolean
     * which in case of true, throws a fatal exception in managedOpen, if no
     * index exists.
     */
    ...
    /**
     * Default value of
     * {@link DidYouMeanSearchNode#CONF_DIDYOUMEAN_ANALYZER},
    ...
    /**
     * Where to place the Did-You-Mean index in the persistent storage.
    ...
    /**
     * Private copy of the local location of the did-you-mean index on disc.
     * Not used when using RAMDirectory.
     */
    private File didyoumeanIndex;

    /**
     * True if we are creating index, false otherwise. Used to stop
     * searching when creating index.
     */
    private boolean creatingIndex = true;

    /** If true, we close when we find no index. */
    private boolean closeOnNonExistingIndex;
    /** Private {@link NgramTokenSuggester}. */
    private NgramTokenSuggester tokenSuggester;
    /** Private aprioriIndex factory. */
    private IndexFacade aprioriIndexFactory;
    /**
    ...
    /**
     * Create needed local indexes: AprioriIndex, TokenSuggester and
    ...
    private void closeIndexes() throws RemoteException {
        try {
            ...
            throw new RemoteException(
                    "IOException while closing DidYouMean index", e);
        }
    }
