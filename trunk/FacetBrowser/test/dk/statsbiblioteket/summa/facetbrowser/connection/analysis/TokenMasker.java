/* $Id: TokenMasker.java,v 1.4 2007/10/11 12:56:25 te Exp $
 * $Revision: 1.4 $
 ...
 * CVS:  $Id: TokenMasker.java,v 1.4 2007/10/11 12:56:25 te Exp $
 */
 ...
 * Typically this is tokens that contains characters that would  be removed, transliterated or make the queryParser cough
 * examples are: c++, c# c*-algebra
 */
...
public class TokenMasker extends Reader {
    ...
    private String defaultMask = "'c++' > 'cplusplus';'c#' > 'csharp'; 'c* algebra' > 'cstaralgebra'; 'c*-algebra > 'cstaralgebra'";
     ...
     * @param keepDefault - keep the default rules (add the new rules to the exsisting add)
     * @param ignoreCase - treat all text as lowercase
     * @throws IOException - if something worng when reading the rules.
     */
    public TokenMasker(Reader reader, String rules, boolean keepDefault, boolean ignoreCase) throws IOException {
        if (keepDefault) {
     ...
     */
    public TokenMasker(TokenStream input, String[] maskRules, boolean ignoreCase) {
        super(input);
        this.ignoreCase = ignoreCase;
        this.maskToken = makeMaskToken(maskRules, ignoreCase);
    }

    private Map<String, String> makeMaskToken(String[] maskRules, boolean ignoreCase) {
        String rules = "";
    ...
    /**
     * Generate a TokenMasker on an tokenStream applying the provided maskMap as rules.
     *
     * @param input - the tokenStream to mask
     * @param maskToken - the map defining maskRules
     * @param ignoreCase - treat text as lowercase.
     */
    public TokenMasker(TokenStream input, Map<String, String> maskToken, boolean ignoreCase) {
        super(input);
        this.ignoreCase = ignoreCase;
