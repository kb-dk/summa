 ...
 * 000000001 24500 L $$aKemiske stoffer$$canvendelse og kontrol$$eudarb. af Byggesektorgruppen på den
 ...
 * 000000001 71000 L $$aRoskilde Universitetscenter$$c<<Den >>teknologisk-samfundsvidenskabelige
 * planlæggeruddannelse$$cByggesektorgruppen
 * 000000001 BAS   L 30
...
public class Aleph2XML2 extends ObjectFilterImpl {
     ...
     * Optional. Default is "-".
     */
    public static final String CONF_ID_DELIMITER = "summa.aleph2xml.iddelimiter";
    ...
    protected boolean processPayload(Payload payload) throws PayloadException {
        ...
        log.debug("Wrapping the Stream in " + payload + " in a Aleph2XMLInputStream");
        try {
            payload.setStream(new Aleph2XMLInputStream(payload.getStream(), payload.toString(), inputCharset));
        } catch (UnsupportedEncodingException e) {
            throw new PayloadException("The charset " + inputCharset + " is not supported by this JVM", e, payload);
        }
        return true;
    }
    ...
    class Aleph2XMLInputStream extends InputStream {
        ...
        private void processLineContent(String line) {
            ...
            if (strparts.length != 4) {
                log.debug("Received unexpected string " + line + " with last id " + lastAlephID);
                return;
            }
            ...
            if (log.isTraceEnabled()) {
                log.trace("Processing line for aleph-ID '" + alephID + "': " + line);
            }
            ...
            if (fieldTag.length() >= 3) {
                if (fieldTag.length() > 5) {
                    ...
                    log.debug("Skipping unknown fieldTag '" + fieldTag + "' from line '" + line + "' from " + debugID);
                }
        ...
        private void makeDatafield(String[] tokens, String fieldTag, String ind1, String ind2, boolean inCAT) {
        ...
        private void finishRecord() {
            if (recordBuffer.length() <= HEADER.length()) {
                ...
                if (isOK) {
                    log.debug("Adding content of record buffer for " + lastAlephID + " to main buffer");
...
/*
    public void applyFilter(File input, Extension ext, String encoding) {
        ...
        try{
            ...
            .append(id).append('-').append(idex).append(SUBFIELD_END).append("\n").append(DATA_END);
            record.append(RECORD_END);
            id = "";
            if (isOK){
                sb.append(record);
